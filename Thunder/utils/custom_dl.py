...
class ByteStreamer:
    ...
    async def get_message_from_group(self, group_id: int, message_id: int) -> Message:
        """Fetch a message from a group (for gallery feature)."""
        while True:
            try:
                message = await self.client.get_messages(group_id, message_id)
                break
            except FloodWait as e:
                logger.debug(f"FloodWait: get_message_from_group, sleep {e.value}s")
                await asyncio.sleep(e.value)
            except Exception as e:
                logger.debug(f"Error fetching message {message_id} from group {group_id}: {e}", exc_info=True)
                raise FileNotFound(f"Message {message_id} not found in group") from e

        if not message or not message.media:
            raise FileNotFound(f"Message {message_id} not found or has no media")
        return message

    async def get_file_info_from_group(self, group_id: int, message_id: int) -> Dict[str, Any]:
        """Get file info from a group message (for gallery feature)."""
        try:
            message = await self.get_message_from_group(group_id, message_id)
            return self.get_file_info_sync(message)
        except Exception as e:
            logger.debug(f"Error getting file info from group for {message_id}: {e}", exc_info=True)
            return {"message_id": message_id, "error": str(e)}

    async def stream_file_from_group(
        self, group_id: int, message_id: int, offset: int = 0, limit: int = 0
    ) -> AsyncGenerator[bytes, None]:
        """Stream file from a group message (for gallery feature)."""
        message = await self.get_message_from_group(group_id, message_id)
        ...
        while True:
            try:
                ...
                logger.debug(f"FloodWait: stream_file_from_group, sleep {e.value}s")
                await asyncio.sleep(e.value)
