...
async def initialize_clients():
    ...
    # Force peer resolution for BIN_CHANNEL
    try:
        print(f"   ▶ Resolving BIN_CHANNEL ({Var.BIN_CHANNEL})...")
        chat = await StreamBot.get_chat(Var.BIN_CHANNEL)
        print(f"   ✓ BIN_CHANNEL resolved: {chat.title}")
    except Exception as e:
        logger.error(f"   ✖ Failed to resolve BIN_CHANNEL: {type(e).__name__}: {e}", exc_info=True)
        print(f"   ⚠ Warning: Failed to resolve BIN_CHANNEL: {e}")
    try:
