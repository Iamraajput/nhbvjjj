...
async def fwd_media(m_msg: Message) -> Optional[Message]:
    try:
        # Attempt to ensure peer is resolved
        try:
            await StreamBot.resolve_peer(Var.BIN_CHANNEL)
        except Exception as e:
            logger.debug(f"Initial resolve_peer failed for {Var.BIN_CHANNEL}: {e}")
        ...
        logger.error(f"Error fwd_media copy to {Var.BIN_CHANNEL}: {e}", exc_info=True)
        if "MEDIA_CAPTION_TOO_LONG" in str(e):
            logger.debug(f"MEDIA_CAPTION_TOO_LONG error, retrying without caption")
            try:
...
async def process_single(
    ...
    try:
        ...
        if source_msg.from_user:
            source_info = f"{source_msg.from_user.first_name} {source_msg.from_user.last_name or ''}".strip()
            if not source_info:
