...
CONFIG_FILE = os.path.join(os.getcwd(), "config.env")
if os.path.exists(CONFIG_FILE):
    load_dotenv(CONFIG_FILE, override=True)
    print(f"   ▶ Loaded config from {CONFIG_FILE}")
else:
    load_dotenv(override=True) # Fallback to standard .env

# Monkeypatch pyrogram.utils to support 64-bit channel IDs
import pyrogram.utils
pyrogram.utils.MIN_CHANNEL_ID = -100999999999999 
print("   ▶ Applied 64-bit channel ID monkeypatch")
...
class Var:
    ...
    if not BIN_CHANNEL:
        logger.debug(f"DEBUG: BIN_CHANNEL from env: {os.getenv('BIN_CHANNEL')}")
    ...
    # Prioritize BASE_URL if provided, otherwise construct from FQDN
    ...
    GALLERY_ENABLED: bool = str_to_bool(os.getenv("GALLERY_ENABLED", "False"))
    GALLERY_TITLE: str = os.getenv("GALLERY_TITLE", "Video Gallery")
    
    GROUP_DATABASE_ID: Optional[int] = None
    group_db_env = os.getenv("GROUP_DATABASE_ID", "").strip()
    if group_db_env:
        try:
            GROUP_DATABASE_ID = int(group_db_env)
        except ValueError:
            logger.warning(f"Invalid GROUP_DATABASE_ID '{group_db_env}' in environment; must be an integer.")
