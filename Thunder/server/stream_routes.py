...
@routes.get(r"/group/{group_id:-?\d+}/{message_id:\d+}/{filename:.+}", allow_head=True)
async def group_media_delivery(request: web.Request):
    """Stream media directly from a group (for gallery feature)."""
    try:
        group_id = int(request.match_info["group_id"])
        message_id = int(request.match_info["message_id"])
        logger.info(f"Group media delivery: group_id={group_id}, message_id={message_id}")
        
        client_id, streamer = select_optimal_client()
        work_loads[client_id] += 1
        
        try:
            file_info = await streamer.get_file_info_from_group(group_id, message_id)
            logger.info(f"File info: {file_info}")
            if not file_info.get('unique_id'):
                logger.error(f"No unique_id in file_info: {file_info}")
            ...
            async def stream_generator():
                try:
                    ...
                    async for chunk in streamer.stream_file_from_group(
                            group_id, message_id, offset=start, limit=content_length):
                        if bytes_to_skip > 0:
        ...
        logger.debug(f"Client error: FileNotFound - {e}", exc_info=True)
        raise web.HTTPNotFound(text="Resource not found") from e
    except Exception as e:
        error_id = secrets.token_hex(6)
...
async def media_delivery(request: web.Request):
    try:
        path = request.match_info["path"]
        logger.info(f"Media delivery called with path: {path}")
        message_id, secure_hash = parse_media_request(path, request.query)
